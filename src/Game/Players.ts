...
export default class Players {
  ...
  public async boot() {
    let playerList = await this.app.serverFacade.client.gbx.query('GetPlayerList', [-1, 0, 1]);

    for (let data:any in playerList) {
   ...
   * @returns {Promise<{}>}
   */
  public async details (player) : any { // todo: declare the interface of returning value type.
   ...
   */
  public async connect(login: string, nickname: string, info?: PlayerInfo | boolean, emit?: boolean) : Player {
    info = info || {};
    emit = emit || false;
